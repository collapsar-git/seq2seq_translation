# ============================
# 改进版 Seq2Seq + Attention 翻译模型
# 关键修复：
# 1. BiLSTM Encoder
# 2. Attention padding mask
# 3. 合理 Dropout (0.2)
# 4. Teacher Forcing 线性衰减
# 5. Beam Search 长度惩罚
# ============================
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src, trg, tf_ratio):
        ...
        for t in range(1, trg_len):
            out, hidden, cell = self.decoder(input, hidden, cell, encoder_outputs, mask)
            outputs[t] = out
            top1 = out.argmax(1)
...
# ---------------- Translation ----------------
def translate(model, sentence, en2id, zh2id, zh_wl, max_len=50):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for _ in range(max_len):
            ...
            if top == zh2id['<eos>']: break
            result.append(zh_wl[top])
...
if __name__ == '__main__':
    ...
    for epoch in range(10):
        start = time.time()
        tf_ratio = max(0.1, 1 - epoch / 10)
        train_loss = run_epoch(model, train_loader, optimizer, criterion, tf_ratio, desc=f'Train Epoch {epoch+1}')
        val_loss = run_epoch(model, dev_loader, optimizer, criterion, desc=f'Val Epoch {epoch+1}')
        ...
        if val_loss < best_val:
            best_val = val_loss
            torch.save(model.state_dict(), 'best_model_attention_2.pt')
    ...
    model.load_state_dict(torch.load('best_model_attention_2.pt'))
